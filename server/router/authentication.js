...
const createAdminToken = () => {
  return jwt.sign({ isAdmin: true }, process.env.JWT_SECRET, { expiresIn: '1d' });
};
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (email === process.env.ADMIN_EMAIL && password === process.env.ADMIN_PASSWORD) {
      req.session.userData = { name: "Admin", email: "admin@gmail.com" };
      res.cookie('userData', { name: "Admin", email: "admin@gmail.com" }, { httpOnly: true });
      console.log("Admin login successful");
      const token = createToken("507f1f77bcf86cd799439011"); // "admin123" is now a string literal
